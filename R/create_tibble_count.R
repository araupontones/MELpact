...
#' @param group_por Variable to which the summarise will be conducted (i.e. count by country or by component)
...
tibble_cuenta_achieved <- function(db, group_por, by_quarter = F, by_country = F, filter_country =" ", ...){
  ...
  data_chart = db %>%
    ...
    group_by_(group_por, "Element", "Indicator") %>%

    #do the counting
    summarise(Total = sum(Approved, na.rm = T), .groups = "drop") %>%
    pivot_wider(id_cols = c(group_por, "Element", "Indicator"),
                names_from = all_of(group_por),
                values_from = Total
    ) %>%
    rowwise() %>%
